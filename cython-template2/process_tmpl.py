...
When used to locate source files as a main program:
    The template files must have an extension '.in'.
    The generated files have the name same as the src file but with the '.in'
    extension removed.
    Example: `wrapper_tmpl.pyx.in` is generated into `wrapper_tmpl.pyx`
...
def main(paths=None):
    ...
    `args` is a list of `.in` template files to convert
